...
async function searchWithRipgrep(
  ...
  try {
    ...
    const results: Record<string, WordMatch[]> = {};
    let totalMatches = 0;
    ...
    if (limitReached) {
      console.error(
        `Warning: Search limited to ${maxResults} results. Use more specific search terms for complete results.`
      );
    }
 ...
 */
async function searchReferencesWithAST(
  directory: string,
  symbol: string,
  maxResults: number = 10
  ...
  try {
    ...
    const results: Record<string, ReferenceMatch[]> = {};
    let totalMatches = 0;
    let limitReached = false;
    ...
    if (limitReached) {
      console.error(
        `Warning: AST search limited to ${maxResults} results. Use more specific search terms for complete results.`
      );
    }

    const fileResults: FileReferenceResult[] = Object.entries(results).map(
      ([file, references]) => ({
        file,
        references,
      })
...
server.tool(
  "search-word",
  "Search for a word in the codebase using ripgrep",
  {
...
server.tool(
  "get_references",
  "Get references to a symbol in the codebase using AST parsing for accurate results",
  {
    symbol: z
      .string()
